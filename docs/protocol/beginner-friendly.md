...
In case of error, we set the `status` field to "error" and we add a new field `error` with two subfields:
code (string): A standard code for the error
message (string): Further information on the error
...
In case of errors, `body` must not be used.
...
To simplify Alice's job, Bob can support an endpoint `/wellknown`, which returns a JSON with two fields:

- `supported`: A dictionary of supported protocols
- `unsupported`: A dictionary of unsupported protocols
  Each element in `supported` has the fields:
- sources (mandatory): A non-empty list of protocol sources
- comment (optional): An optional comment specifying the rationale for supporting the protocol
  For `unsupported`, the fields are the same, but both `sources` and `comment` are optional.
